...
import {
  BookOpen, Clock, TrendingUp, ArrowRight, Search,
  X, ArrowLeft, ExternalLink, Share2, Filter, Tag,
} from 'lucide-react';
...
if (typeof document !== 'undefined' && !document.getElementById('aw-resources-styles')) {
...
export const PublicResourcesPage: React.FC<PublicResourcesPageProps> = ({ onNavigate }) => {
  ...
  const loadArticleBySlug = async (slug: string) => {
    try {
      ...
      if (data) { setSelected(data); incrementViews(data.id); }
    } catch (err) { console.error(err); }
  };

  const incrementViews = async (id: string) => {
    try { await supabase.rpc('increment_article_view_count', { article_id: id }); } catch {}
  };
    ...
    return (
      <div style={pageStyle}>
          ...
          <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <button
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: T.textMuted, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', transition: 'color 0.2s' }}
        ...
        <article style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px 80px' }}>
          ...
          {/* Title */}
          <h1 style={{ fontSize: 'clamp(26px,4vw,42px)', fontWeight: 900, color: T.white, lineHeight: 1.2, letterSpacing: '-0.5px', marginBottom: 16 }}>
            {selectedArticle.title}
          </h1>

          {/* Meta description */}
          <p style={{ fontSize: 18, color: T.textBody, lineHeight: '30px', marginBottom: 32, borderLeft: `3px solid ${T.accent}`, paddingLeft: 16 }}>
            {selectedArticle.meta_description}
          </p>

          {/* Featured image */}
          {selectedArticle.featured_image_url && (
            <div style={{ borderRadius: 12, overflow: 'hidden', marginBottom: 36, border: `1px solid ${T.border}` }}>
              <img src={selectedArticle.featured_image_url} alt={selectedArticle.title} style={{ width: '100%', height: 'auto', display: 'block' }} />
            </div>
          )}
          ...
          {/* Article CTA */}
          <div style={{ marginTop: 48, background: 'rgba(200,255,0,0.04)', border: '1px solid rgba(200,255,0,0.18)', borderRadius: 14, padding: '36px', textAlign: 'center' }}>
            ...
            <button
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 32px', background: T.accent, color: T.accentDark, fontSize: 15, fontWeight: 700, borderRadius: 10, border: 'none', cursor: 'pointer', boxShadow: '0 0 20px rgba(200,255,0,0.25)', transition: 'opacity 0.2s, transform 0.15s', fontFamily: 'inherit' }}
  ...
  return (
    <div style={pageStyle}>
      ...
      {/* ── Hero banner ── */}
      <div style={{ position: 'relative', padding: '96px 24px 80px', overflow: 'hidden' }}>
        {/* Glow */}
        ...
        <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center', position: 'relative' }}>
          ...
          {/* Badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', background: 'rgba(200,255,0,0.08)', border: '1px solid rgba(200,255,0,0.20)', borderRadius: 9999, marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 17, color: T.textBody, lineHeight: '28px', maxWidth: 560, margin: '0 auto' }}>
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '0 24px 96px' }}>
        ...
        <div style={{ marginBottom: 40, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ flex: 1, minWidth: 240, position: 'relative' }}>
              ...
              {searchTerm && (
                <button
                  ...
                  style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: T.textMuted, display: 'flex', padding: 2, transition: 'color 0.2s' }}
                  onMouseEnter={e => (e.currentTarget.style.color = T.white)}
                  onMouseLeave={e => (e.currentTarget.style.color = T.textMuted)}
                >
                  <X size={15} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <p style={{ fontSize: 13, color: T.textMuted }}>
              <span style={{ color: T.accent, fontWeight: 700 }}>{filtered.length}</span> {filtered.length === 1 ? 'article' : 'articles'} found
              {selectedCategory !== 'all' && <span style={{ color: T.textMuted }}> in <span style={{ color: T.textLabel }}>{selectedCategory}</span></span>}
            </p>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <button
              onClick={() => { setSearchTerm(''); setCategory('all'); }}
              style={{ padding: '10px 24px', background: T.accent, color: T.accentDark, fontSize: 14, fontWeight: 700, borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
          ...
          <div className="aw-articles-grid">
            {filtered.map(article => (
              ...
              >
                {/* Thumbnail */}
                <div style={{ height: 180, background: 'rgba(255,255,255,0.04)', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                  {article.featured_image_url ? (
                    ...
                    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, rgba(200,255,0,0.05) 0%, rgba(255,255,255,0.02) 100%)' }}>
                      <BookOpen size={32} style={{ color: 'rgba(200,255,0,0.25)' }} />
                    </div>
                  )}
                  {/* Category overlay */}
                ...
                <div style={{ padding: '20px 22px 22px', flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <p style={{ fontSize: 13, color: T.textBody, lineHeight: '21px', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {article.excerpt}
                  </p>
                  ...
                  {/* Read link */}
                  <div style={{ marginTop: 'auto', paddingTop: 8, borderTop: `1px solid rgba(255,255,255,0.05)`, display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: T.accent }}>
                    Read Article
        ...
        {!loading && filtered.length > 0 && (
          <div style={{ marginTop: 64, background: 'rgba(200,255,0,0.03)', border: '1px solid rgba(200,255,0,0.14)', borderRadius: 16, padding: '48px 40px', textAlign: 'center' }}>
