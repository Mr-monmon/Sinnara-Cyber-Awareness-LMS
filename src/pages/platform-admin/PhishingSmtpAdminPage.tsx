...
export const PhishingSmtpAdminPage: React.FC = () => {
  ...
  const revokeCompany = async (companyId: string) => {
    ...
    try {
      ...
      setPushStatus(next);
      // If this was the last grant on a SHARED profile, fail closed → PLATFORM_ONLY.
      if (Object.keys(next).length === 0 && pushModal.profile.visibility === 'SHARED') {
  ...
  const pushToSelected = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Sharing with selected companies → visibility SHARED (required for the
      // server-side accessibility check to grant those companies access).
  ...
  const pushToAll = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('smtp_profile_company_access').upsert(rows, { onConflict: 'smtp_profile_id,company_id' });
      if (error) throw error;
      // Visible to every company → mark GLOBAL (access rows kept for the UI count).
  ...
  // Restrict a profile to platform admins only: clear all company grants and
  // set visibility PLATFORM_ONLY (no company can use it after this).
  const makePlatformOnly = async () => {
    if (!pushModal.profile) return;
    if (!confirm('Make this profile platform-only? All companies will lose access to it.')) return;
    setPushing(true);
    try {
      ...
      setSelectedCompanies(new Set());
      loadAll();
    } catch (err) { console.error('[SmtpAdmin] platformOnly', err); alert('Update failed: ' + getErrorMessage(err)); }
    finally { setPushing(false); }
  };
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div className="aw-modal-in"
            ...
            <div style={{ padding: '14px 22px', borderTop: `1px solid ${T.borderFaint}`, display: 'flex', gap: 10, flexShrink: 0 }}>
              ...
              <button onClick={makePlatformOnly} disabled={pushing} title="Restrict to platform admins only" style={{ padding: '10px 16px', borderRadius: 10, background: T.redBg, border: `1px solid ${T.redBorder}`, color: T.red, cursor: pushing ? 'not-allowed' : 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: 13 }}>Platform Only</button>
              <button onClick={pushToAll} disabled={pushing} title="Visible to all companies (GLOBAL)" style={{ padding: '10px 16px', borderRadius: 10, background: T.orangeBg, border: `1px solid ${T.orangeBorder}`, color: T.orange, cursor: pushing ? 'not-allowed' : 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: 13 }}>Push to All</button>
