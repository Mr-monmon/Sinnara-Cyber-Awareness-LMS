...
/* ─────────────────────────────────────────
   DEFAULTS
───────────────────────────────────────── */
const defaultExamForm = { title: '', description: '', exam_type: 'GENERAL' as any, passing_score: 70, time_limit_minutes: 30 };
const defaultQForm    = { question: '', option1: '', option2: '', option3: '', option4: '', correct_answer: '', explanation: '' };
...
export const ExamsPage: React.FC = () => {
  ...
  const handleEdit = (exam: Exam) => {
    setEditingExam(exam);
    setForm({ title: exam.title, description: exam.description, exam_type: exam.exam_type, passing_score: exam.passing_score, time_limit_minutes: exam.time_limit_minutes || 30 });
    setShowModal(true);
  };
  ...
  const handleAddQ = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('exam_questions').insert([{
        exam_id: selectedExam.id, question: qForm.question,
        options, correct_answer: qForm.correct_answer,
        explanation: qForm.explanation, order_index: questions.length,
      }]);
      if (error) throw error;
      setQForm({ ...defaultQForm });
      const { data } = await supabase.from('exam_questions').select('*').eq('exam_id', selectedExam.id).order('order_index');
      if (data) setQuestions(data);
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <div className="aw-fade-up" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 5 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 900, color: T.white, letterSpacing: '-0.3px', margin: 0 }}>Exams & Assessments</h1>
          </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(10,12,6,0.86)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)' }}
          ...
          <div className="aw-modal-in aw-exp-scroll" style={{ width: '100%', maxWidth: 480, background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 16, overflow: 'hidden', overflowY: 'auto', maxHeight: '90vh', boxShadow: '0 40px 100px rgba(0,0,0,0.60)', fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column' }}
            ...
            <form onSubmit={handleSubmit} style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
                ...
                <div>
                  ...
                  <input className="aw-exp-input" type="text" required placeholder="e.g. Cybersecurity Awareness Quiz" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
                </div>

                <div>
                  <label className="aw-exp-label">Description <span style={{ color: T.accent }}>*</span></label>
                  <textarea className="aw-exp-textarea" required rows={3} placeholder="Brief exam description" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
                </div>
                    ...
                    {Object.entries(EXAM_TYPE_CFG).map(([key, cfg]) => {
                      ...
                      return (
                        <button key={key} type="button"
                          ...
                          onClick={() => setForm(p => ({ ...p, exam_type: key as any }))}>
                          {cfg.label}
                        </button>
                      );
                    })}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    ...
                    <input className="aw-exp-input" type="number" required min="0" max="100" value={form.passing_score} onChange={e => setForm(p => ({ ...p, passing_score: parseInt(e.target.value) || 0 }))} />
                  </div>
                  <div>
                    <label className="aw-exp-label">Time Limit (min) <span style={{ color: T.textMuted, fontWeight: 400 }}>(optional)</span></label>
                    <input className="aw-exp-input" type="number" min="1" value={form.time_limit_minutes} onChange={e => setForm(p => ({ ...p, time_limit_minutes: parseInt(e.target.value) || 0 }))} />
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding: '14px 22px', borderTop: `1px solid ${T.borderFaint}`, display: 'flex', gap: 10, flexShrink: 0 }}>
                ...
                <button type="submit" className="aw-exp-save-btn" disabled={saving}>
                  {saving ? <><Loader2 size={14} style={{ animation: 'aw-spin 0.8s linear infinite' }} /> Saving…</> : <><Save size={14} /> {editingExam ? 'Update Exam' : 'Create Exam'}</>}
                </button>
              </div>
            </form>
          </div>
      ...
      {showQModal && selectedExam && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(10,12,6,0.88)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)' }}
          ...
          <div className="aw-modal-in" style={{ width: '100%', maxWidth: 900, background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 18, overflow: 'hidden', maxHeight: '94vh', boxShadow: '0 40px 100px rgba(0,0,0,0.65)', fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column' }}
            onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleAddQ} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  <div className="aw-exp-scroll" style={{ overflowY: 'auto', flex: 1, padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 13 }}>
                    ...
                    <div>
                      ...
                      <textarea className="aw-exp-textarea" required rows={3} placeholder="Enter your question here…" value={qForm.question} onChange={e => setQForm(p => ({ ...p, question: e.target.value }))} />
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      {(['option1','Option 1 *'],['option2','Option 2 *'],['option3','Option 3'],['option4','Option 4']) as [string,string][]).map(([key, label]) => (
                        <div key={key}>
                          <label className="aw-exp-label">{label}</label>
                          <input className="aw-exp-input" type="text" required={key === 'option1' || key === 'option2'} value={(qForm as any)[key]} onChange={e => setQForm(p => ({ ...p, [key]: e.target.value }))} style={{ fontSize: 13 }} />
                        </div>
                      ))}
                    </div>
                    ...
                    <div>
                      ...
                      {[qForm.option1, qForm.option2, qForm.option3, qForm.option4].filter(o => o.trim()).length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {[qForm.option1, qForm.option2, qForm.option3, qForm.option4].filter(o => o.trim()).map((opt, i) => (
                            <div key={i} className={`aw-exp-opt-radio ${qForm.correct_answer === opt ? 'correct' : ''}`} onClick={() => setQForm(p => ({ ...p, correct_answer: opt }))}>
                        ...
                        <p style={{ fontSize: 12, color: T.textMuted, padding: '8px 0' }}>Fill in the options above first</p>
                      )}
                    </div>

                    <div>
                      <label className="aw-exp-label">Explanation <span style={{ color: T.textMuted, fontWeight: 400 }}>(optional)</span></label>
                      <textarea className="aw-exp-textarea" rows={2} placeholder="Why is this the correct answer?" value={qForm.explanation} onChange={e => setQForm(p => ({ ...p, explanation: e.target.value }))} />
                    </div>
                  </div>
                  ...
                  <div style={{ padding: '12px 20px', borderTop: `1px solid ${T.borderFaint}`, flexShrink: 0 }}>
                    <button type="submit" className="aw-exp-add-q-btn" disabled={addingQ}>
                      {addingQ ? <><Loader2 size={13} style={{ animation: 'aw-spin 0.8s linear infinite' }} /> Adding…</> : <><Plus size={13} /> Add Question</>}
                    </button>
                  </div>
                </form>
              </div>
                ...
                <div className="aw-exp-scroll" style={{ overflowY: 'auto', flex: 1, padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 9 }}>
                  {questions.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '40px 0', color: T.textMuted, fontSize: 13 }}>
                      <ClipboardCheck size={32} style={{ color: 'rgba(255,255,255,0.10)', margin: '0 auto 12px', display: 'block' }} />
                      ...
                      <div key={q.id} className="aw-exp-q-card">
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
                          <span style={{ fontSize: 13, fontWeight: 700, color: T.white, lineHeight: '20px' }}>
                            <span style={{ color: T.textMuted, marginRight: 6, fontWeight: 400 }}>{idx + 1}.</span>{q.question}
                          </span>
