...
if (
  ...
  !document.getElementById("aw-emp-styles")
) {
...
export const EmployeesPage: React.FC<EmployeesPageProps> = ({
    ...
    try {
      if (editingEmployee) {
        ...
        await sendEmail(
          ...
          "Welcome to Awareone",
          "Welcome aboard",
          "We created your employee account. Use the credentials below to sign in.",
          loginUrl
        );
      }
      setShowModal(false);
  ...
  const uploadCSV = async (file?: File) => {
    if (!currentUser?.company_id) {
      setUploadError("Missing company context.");
      return;
    }
    if (!file) {
    ...
    if (
      ...
      setUploadError("Selected file is not a CSV.");
      return;
    }
    setUploadError("");
    setUploadSummary("");
    ...
    try {
      ...
      setUploadError(
        "Failed to parse or upload CSV. Please check the file and try again."
      );
    } finally {
              ...
              <tr key={emp.id}>
                ...
                <td>
                  ...
                  >
                    {onViewEmployee && (
                      <button
                        className="aw-emp-icon-btn view"
                        title="View Details"
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleDelete(emp.id, emp.email, emp.full_name)
                      }
                    >
                      <Trash2 size={13} />
                    </button>
              ...
              >
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: T.textMuted, marginTop: 5 }}>
                    Assigning a department auto-enrolls the employee in
                    department courses.
                  </p>
                </div>
