...
import {
  Send, Calendar, Users, ClipboardList, AlertCircle,
  Undo2, Mail, Plus, X, Save, Loader2, Building,
...
export const ExamAssignmentPage: React.FC = () => {
  ...
  const [form, setForm] = useState({
    exam_id: '', assignment_type: 'employee', target_id: '',
    due_date: '', max_attempts: 1, is_mandatory: true,
  });

  useEffect(() => {
    loadExams(); loadEmployees(); loadDepartments(); loadAssignments();
  }, [user]);
  ...
  const loadAssignments = async () => {
    ...
    if (error) { setError('Failed to load assignments: ' + error.message); return; }
    ...
    const passedIds = new Set((resultsRes.data || []).map(r => r.assignment_id));

    const enriched = await Promise.all(rawData.map(async a => {
      let actualStatus = a.status;
      if (passedIds.has(a.id) && actualStatus === 'active') {
        actualStatus = 'completed';
        await supabase.from('assigned_exams').update({ status: 'completed' }).eq('id', a.id);
      }
      return {
        ...a, status: actualStatus,
        exams: examsMap.get(a.exam_id) || null,
        assigned_to_employee: a.assigned_to_employee ? usersMap.get(a.assigned_to_employee) || null : null,
        assigned_to_department: a.assigned_to_department ? deptsMap.get(a.assigned_to_department) || null : null,
      };
    }));
  ...
  const handleAssign = async (e: React.FormEvent) => {
    ...
    try {
      const assignment: any = {
        exam_id: form.exam_id, company_id: user?.company_id, assigned_by: user?.id,
        due_date: form.due_date || null, max_attempts: form.max_attempts,
        is_mandatory: form.is_mandatory, status: 'active',
      };
      if (form.assignment_type === 'employee') assignment.assigned_to_employee = form.target_id;
      else assignment.assigned_to_department = form.target_id;
      const { error: insertError } = await supabase.from('assigned_exams').insert(assignment);
      if (insertError) {
        setError(insertError.code === '23505' ? 'This exam is already assigned to the selected target.' : 'Failed to assign exam: ' + insertError.message);
        return;
      }
      setShowModal(false);
      setForm({ exam_id: '', assignment_type: 'employee', target_id: '', due_date: '', max_attempts: 1, is_mandatory: true });
      loadAssignments();
    } finally { setSaving(false); }
  };

  const handleWithdraw = async (id: string, examTitle: string, targetName: string) => {
    if (!confirm(`Withdraw "${examTitle}" from ${targetName}?\n\nThe exam will no longer be visible but attempt history will be preserved.`)) return;
    ...
    if (error) { setError('Error withdrawing: ' + error.message); return; }
    loadAssignments();
  };
  ...
  const handleReminder = async (assignment: AssignedExam) => {
    ...
    const dueTxt = assignment.due_date ? ` Please complete it before ${new Date(assignment.due_date).toLocaleDateString()}.` : '';
    let recipients: Array<{ email: string; full_name: string }> = [];
    let targetName = 'this assignee';
    if (assignment.assigned_to_employee?.email) {
      recipients = [{ email: assignment.assigned_to_employee.email, full_name: assignment.assigned_to_employee.full_name }];
      targetName = assignment.assigned_to_employee.full_name;
    } else if (assignment.assigned_to_department?.id) {
      recipients = employees.filter(emp => emp.department_id === assignment.assigned_to_department?.id && emp.email).map(emp => ({ email: emp.email, full_name: emp.full_name }));
      targetName = `Dept: ${assignment.assigned_to_department.name}`;
    }
    if (!recipients.length) { setError('No email recipients found for this assignment.'); return; }
    if (!confirm(`Send ${recipients.length > 1 ? 'reminders' : 'a reminder'} for "${examTitle}" to ${targetName}?`)) return;
    try {
      const results = await Promise.all(recipients.map(r => sendEmail(r.email, r.full_name, `Exam Reminder: ${examTitle}`, 'Exam Reminder', `This is a reminder that "${examTitle}" is still assigned to you.${dueTxt} Please log in to complete it.`)));
      const failed = results.find(r => r.error);
      if (failed?.error) throw failed.error;
      alert(recipients.length > 1 ? `Reminders sent to ${recipients.length} employees.` : 'Reminder sent successfully.');
    } catch { setError('Failed to send reminder email.'); }
  };
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="aw-fade-up" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: T.textBody, margin: 0 }}>Assign exams to individual employees or entire departments.</p>
        </div>
        <button
        ...
        <div style={{ overflowX: 'auto' }} className="aw-ea-scroll">
          <table className="aw-ea-table">
            ...
            <tbody>
              {assignments.map(a => {
                ...
                const Icon     = cfg.icon;
                const isTarget = a.assigned_to_employee?.full_name ?? (a.assigned_to_department?.name ? `Dept: ${a.assigned_to_department.name}` : null);
                const targetEl = a.assigned_to_employee?.full_name ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: T.blueBg, border: `1px solid ${T.blueBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Users size={12} style={{ color: T.blue }} />
                    </div>
                    <div>
                      <div style={{ fontWeight: 600, color: T.white }}>{a.assigned_to_employee.full_name}</div>
                      <div style={{ fontSize: 11, color: T.textMuted }}>{a.assigned_to_employee.email}</div>
                    </div>
                  </div>
                ...
                return (
                  <tr key={a.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <ClipboardList size={14} style={{ color: T.blue, flexShrink: 0 }} />
                        <span style={{ fontWeight: 600, color: T.white }}>{a.exams?.title || 'N/A'}</span>
                      </div>
                    </td>
                    <td>{targetEl}</td>
                    ...
                    <td style={{ textAlign: 'center', fontSize: 12, color: T.textMuted }}>
                      {a.due_date ? fmt(a.due_date) : <span style={{ color: T.borderFaint }}>Not set</span>}
                    </td>
                    <td style={{ textAlign: 'center' }}>
                      ...
                      {a.status === 'active' ? (
                        <div style={{ display: 'flex', gap: 7, justifyContent: 'center' }}>
                          ...
                          <button className="aw-ea-icon-btn withdraw" onClick={() => {
                            const name = a.assigned_to_employee?.full_name ?? `Dept: ${a.assigned_to_department?.name || 'Unknown'}`;
                            handleWithdraw(a.id, a.exams?.title || 'Unknown Exam', name);
                          }}>
                            <Undo2 size={13} />
          ...
          {assignments.length === 0 && (
            <div style={{ textAlign: 'center', padding: '48px 0' }}>
              ...
              <p style={{ fontSize: 12, color: T.textMuted, opacity: 0.7, margin: '6px 0 18px' }}>Click "Assign Exam" to get started</p>
              <button onClick={() => { setShowModal(true); setError(null); }}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 18px', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', background: T.accent, color: T.accentDark }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(10,12,6,0.82)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)' }}
          ...
          <div className="aw-modal-in aw-ea-scroll" style={{ width: '100%', maxWidth: 500, background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 16, overflow: 'hidden', overflowY: 'auto', maxHeight: '92vh', boxShadow: '0 32px 80px rgba(0,0,0,0.55)', fontFamily: "'Inter', sans-serif" }}
            ...
            <div style={{ padding: '18px 22px', borderBottom: `1px solid ${T.borderFaint}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: T.textMuted, margin: 0 }}>Fill in the details below</p>
                </div>
              </div>
            ...
            <form onSubmit={handleAssign}>
              <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div>
                  ...
                  <select className="aw-ea-select" required value={form.exam_id} onChange={e => setForm(p => ({ ...p, exam_id: e.target.value }))}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 9 }}>
                    {[
                      { key: 'employee',   icon: Users,    label: 'Individual Employee' },
                      { key: 'department', icon: Building, label: 'Entire Department'   },
                    ].map(({ key, icon: Icon, label }) => (
                      <div key={key} className={`aw-ea-radio-row ${form.assignment_type === key ? 'sel' : ''}`}
                        onClick={() => setForm(p => ({ ...p, assignment_type: key, target_id: '' }))}>
                        <div className="aw-ea-rdot">{form.assignment_type === key && <div className="aw-ea-rdot-inner" />}</div>
                        <Icon size={13} style={{ color: form.assignment_type === key ? T.accent : T.textMuted }} />
                        <span style={{ fontSize: 12, fontWeight: 600, color: form.assignment_type === key ? T.white : T.textMuted }}>{label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <label className="aw-ea-label">
                    {form.assignment_type === 'employee' ? 'Select Employee' : 'Select Department'} <span style={{ color: T.accent }}>*</span>
                  </label>
                  <select className="aw-ea-select" required value={form.target_id} onChange={e => setForm(p => ({ ...p, target_id: e.target.value }))}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    ...
                    <input className="aw-ea-input" type="number" min="1" max="10" required value={form.max_attempts}
                      onChange={e => setForm(p => ({ ...p, max_attempts: parseInt(e.target.value) || 1 }))} />
                  </div>
                  <div>
                    <label className="aw-ea-label">Due Date <span style={{ color: T.textMuted, fontWeight: 400 }}>(optional)</span></label>
                    <input className="aw-ea-input" type="date" value={form.due_date} onChange={e => setForm(p => ({ ...p, due_date: e.target.value }))} />
                  </div>
                </div>

                {/* Mandatory toggle */}
                <div className={`aw-ea-toggle-row ${form.is_mandatory ? 'on' : ''}`}
                  onClick={() => setForm(p => ({ ...p, is_mandatory: !p.is_mandatory }))}>
                  <div style={{ width: 36, height: 20, borderRadius: 9999, background: form.is_mandatory ? T.green : 'rgba(255,255,255,0.10)', border: `1px solid ${form.is_mandatory ? T.greenBorder : T.borderFaint}`, position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
                    <div style={{ width: 14, height: 14, borderRadius: '50%', background: form.is_mandatory ? T.accentDark : 'rgba(255,255,255,0.40)', position: 'absolute', top: 2, left: form.is_mandatory ? 18 : 2, transition: 'left 0.2s', boxShadow: '0 1px 3px rgba(0,0,0,0.30)' }} />
                  </div>
                  <div>
                    <span style={{ fontSize: 13, fontWeight: 700, color: form.is_mandatory ? T.green : T.textMuted }}>
                      {form.is_mandatory ? 'Mandatory Exam' : 'Optional Exam'}
                    </span>
                    <p style={{ fontSize: 11, color: T.textMuted, margin: 0, marginTop: 2 }}>
                      {form.is_mandatory ? 'Employees must complete this exam' : 'Employees can skip this exam'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding: '14px 22px', borderTop: `1px solid ${T.borderFaint}`, display: 'flex', gap: 10 }}>
                <button type="button" className="aw-ea-cancel-btn" onClick={() => { setShowModal(false); setError(null); }}>Cancel</button>
                <button type="submit" className="aw-ea-save-btn" disabled={saving}>
                  {saving
