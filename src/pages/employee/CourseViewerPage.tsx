...
export const CourseViewerPage: React.FC<CourseViewerProps> = ({
  ...
  const markSectionComplete = async (sectionId: string) => {
    ...
    if (error) {
      ...
      alert(
        t("courseViewer.completionFailed", {
          ns: "employee",
  ...
  const updateCourseProgress = async () => {
    ...
    const completedCount =
      Object.values(progress).filter((p) => p.completed).length + 1;
  ...
  const updateCourseCompletion = async () => {
    ...
    const allCompleted = sections.every(
      (section) => progress[section.id]?.completed
    );
    ...
    if (checkError) {
      ...
      alert(
        t("courseViewer.checkEnrollmentError", {
          ns: "employee",
    ...
    if (certificate) {
      ...
      const { data: courseData } = await supabase
        .from("courses")
        .select("title, certificate_id")
        .eq("id", courseId)
        .maybeSingle();

      const certNumber = `CERT-${new Date().getFullYear()}-${Math.floor(
        Math.random() * 999999
      )
        .toString()
        .padStart(6, "0")}`;
      ...
      await supabase.from("issued_certificates").insert({
        ...
        course_name:
          courseData?.title ||
          t("courseViewer.fallbackCourseName", { ns: "employee" }),
      ...
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <p className="text-slate-600">
            {t("courseViewer.loading", { ns: "employee" })}
          </p>
        </div>
      </div>
    );
  ...
  if (!currentSection && !isLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <p className="text-slate-600">
            {t("courseViewer.noContent", { ns: "employee" })}
          </p>
        </div>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-slate-600">
              {t("courseViewer.progress", { ns: "employee" })}:{" "}
              ...
              {currentSection.section_type === "VIDEO" && (
                <div>
                  <div className="aspect-video bg-slate-900 rounded-lg mb-6 overflow-hidden">
                    {currentSection.content.includes("youtube.com") ||
                    currentSection.content.includes("youtu.be") ? (
                      <iframe
                ...
                <div>
                  {!quizSubmitted ? (
                    <div className="space-y-6">
                      {currentSection.content_data?.questions?.map(
                        (q: any, qIndex: number) => (
                          <div
                            key={qIndex}
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          currentSection.content_data?.questions?.length
                        }
