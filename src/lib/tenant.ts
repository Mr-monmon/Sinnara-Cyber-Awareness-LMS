...
export type HostAccessMode = "apex" | "tenant" | "invalid";
...
export const getHostAccessMode = (hostname: string): HostAccessMode => {
  ...
  if (isLocalhostDomain(hostname)) {
    if (labels.length === 1 || RESERVED_SUBDOMAINS.has(candidateSubdomain)) {
      return "apex";
    }
  ...
  if (RESERVED_SUBDOMAINS.has(candidateSubdomain)) {
    return "apex";
  }
